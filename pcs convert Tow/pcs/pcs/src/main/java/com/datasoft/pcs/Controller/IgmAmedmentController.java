...
public class IgmAmedmentController {
   ...
   @RequestMapping(value = "/ViewIgmDetailsList/{rotation}/{bl_no}/{container_no}", method= RequestMethod.GET)
    ...
    @RequestMapping(value = "/getViewIgmSubDetailContainer/{container_no}/{bl_no}/{rotation}/{igmType}/{priorValue}/{newValue}/{login_id}", method= RequestMethod.POST)
    @ResponseBody
    public List getViewIgmSubDetailContainer(@PathVariable String container_no,@PathVariable String bl_no, @PathVariable String rotation , @PathVariable String igmType,@PathVariable  String priorValue,@PathVariable String newValue,@PathVariable  String login_id
    )  {
        String Rotation=rotation.replace('_','/');
        System.out.println("rafiqul:"+Rotation);
        List list =new ArrayList<>();
        list = igmAmedmentChangeContainerStatusService.getViewIgmSubDetailContainer(container_no,bl_no,Rotation,igmType,priorValue,newValue,login_id);
