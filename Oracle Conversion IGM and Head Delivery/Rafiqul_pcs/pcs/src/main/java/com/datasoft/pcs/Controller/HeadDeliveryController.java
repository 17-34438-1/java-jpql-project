...
public class HeadDeliveryController {
    ...
    //  for searchBeList
    @RequestMapping(value = "/SearchBillList/{orgTypeId}/{logInId}/{SearchCriteria}",method = RequestMethod.GET)
    @ResponseBody
    public List searchBillList(@PathVariable Long orgTypeId,@PathVariable String logInId,@PathVariable String SearchCriteria){
        List list =new ArrayList();
        list=billOfEntryListService.searchBillList(orgTypeId,logInId,SearchCriteria);
        return list;
    }
